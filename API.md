...
JWT includes role claim (`User` | `Admin`). Admin role is assigned server-side when the Telegram user ID is listed in config `Admin:TelegramUserIds` / `ADMIN_TELEGRAM_USER_IDS` (comma-separated). The frontend cannot set the role.

### `GET /api/me` (JWT)
Safe profile. Includes `role` and `isAdmin`. `binolla.connected` reflects **live** session when present. Never returns SSID.
...
### `GET /api/admin/binolla/accounts?status=Pending|Approved|Rejected`
...
Sets `AdminApproved = true`, `ApprovalStatus = Approved`, `ApprovedAt`, `ApprovedBy` (admin identity). Writes audit event. Idempotent if already approved.

### `POST /api/admin/binolla/accounts/{id}/reject`
Sets `AdminApproved = false`, `ApprovalStatus = Rejected`. Does **not** delete the link. Writes audit event. User bot access becomes `NotEligible`.
